...
const EditPost = async (event) =>{
  ...
  document.querySelector('#post-input').value = document.querySelector('#actualPost').innerHTML;
...
if (event.target.hasAttribute('data-id')) {
  ...
  if (response.ok) {
    // document.location.replace(`/post/${postId}`);
      ...
      editArea.style.display = 'none';
      document.querySelector('#actualPost').innerHTML = document.querySelector('#post-input').value;
  } else {
...
document.querySelector('#finish-post').addEventListener('click', ActuallyEditPost);